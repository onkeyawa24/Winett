 <?php 
    ...
    $con = mysqli_connect('localhost', 'root', '', 'wenett');

    if (isset($_POST['liked'])) {
        $postid = $_POST['postid'];
        $result = mysqli_query($con, "SELECT * FROM post_table WHERE post_id=$postid");
        $row = mysqli_fetch_array($result);
        $n = $row['likes'];

        mysqli_query($con, "INSERT INTO reactions (user_id, post_id) VALUES (1, $postid)");
        mysqli_query($con, "UPDATE reactions SET likes=$n+1 WHERE reaction_id=$postid");
